...
const PredictionChart = ({ dateRange, onLoading, onError }) => {
  ...
  const fetchChartData = async () => {
    try {
      ...
      // Fetch batch prediction data from API
      const response = await axios.get('/api/v1/predict/batch', {
        params: { limit: 100 }
      });
    ...
    return (
      <Card className="h-96">
        <div className="h-full flex flex-col items-center justify-center text-center p-6">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">Error Loading Prediction Data</h3>
    ...
    <Card className="rounded-xl shadow-md bg-white p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Model performance visualization over time
        </p>
      </div>
